...
@call_logger
def boot(nodes, frequency=10, session=None, attempts=10):
    """
    ...
    json_response = call(boot_endpoint, nodes, output_format, "BOA: Staging node for boot...")
...
def shutdown(nodes, force=True, frequency=10, session=None, attempts=10):
    ...
    json_response = call(shutdown_endpoint, nodes, output_format, "BOA: Staging nodes for shutdown...",
         force=force)
...
def graceful_shutdown(nodes, grace_window=300, hard_window=180, graceful_prewait=20, 
    ...
    _, _ = shutdown(list(nodes_on), force=False, session=session)
